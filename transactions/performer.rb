module Transactions
  class Performer
    ...
    def run(year)
      data = {}
      fetch(year).each do |row|
        performer = row["performer"]
        data[performer] ||= hash_init()
        data[performer][row["type"]] += row["antal"]
      end

      data.map do |performer, values|
        values["performer"] = performer
        values["sum"] = values["issue"] + values["onsite_checkout"] + values["return"]
        values
      end
    end
